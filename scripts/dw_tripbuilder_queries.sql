...
SELECT f1.id_trajectory FROM fato f1
INNER JOIN fato f2 ON f1.id_trajectory = f2.id_trajectory
INNER JOIN tb_poi p1 ON p1.id = f1.id_category
INNER JOIN tb_poi p2 ON p2.id = f2.id_category
WHERE p1.name = 'Torre_del_Leone' AND p2.name = 'Torre_pendente_di_Pisa'
AND f1.position = f2.position-1 OR f1.position-1 = f2.position
...
INNER JOIN tb_category c2 ON c2.id = f2.id_category
WHERE c1.name = 'museidipisa' AND c2.name = 'cappelledipisa'
AND f1.position = f2.position-1 AND tm = 'Bus'
...
INNER JOIN tb_category c2 ON c2.id = f2.id_category
INNER JOIN tb_category c3 ON c3.id = f3.id_category
WHERE c1.name = 'torridipisa' AND f1.position = 1
AND f2.position = 2 AND tm2.value = 'Walk'
AND f3.position = 3 AND tm3.value = 'Bus'
AND c3.name = 'chiesedipisa' AND c3.position = 4
